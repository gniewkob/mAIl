...
class SqliteBaseRepository:
    ...
    def _row_to_record(self, row: sqlite3.Row) -> EmailRecord:
        ...
        data = dict(row)
        # Handle partial rows by providing defaults for missing keys
        if "mailbox_id" not in data:
            data["mailbox_id"] = "unknown"
        ...
        return EmailRecord(
            ...
            sender=data["sender"],
            sender_sha256=data.get("sender_sha256"),
            subject=data["subject"],
            subject_sha256=data.get("subject_sha256"),
            internaldate=data.get("internaldate"),
            status=WorkflowStatus(data["status"]) if data.get("status") else WorkflowStatus.PROCESSING,
...
class SqliteStateRepository(SqliteBaseRepository, StateRepositoryProtocol):
    ...
    def get_by_id(self, record_id: int) -> EmailRecord | None:
        ...
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM email_processing_state WHERE id = ?", (record_id,))
            row = cursor.fetchone()
            return self._row_to_record(row) if row else None
...
class SqliteWorkerLockRepository(SqliteBaseRepository, WorkerLockRepositoryProtocol):
    ...
    def acquire_worker_lock(
        ...
        with self._connect() as conn:
            ...
            # Ensure worker_runtime_lock table exists
            cursor.execute(
            ...
            cursor.execute(
                """INSERT INTO worker_runtime_lock (lock_name, lock_owner, lock_expires_at, updated_at)
                   ...
                   WHERE worker_runtime_lock.lock_expires_at < ? OR worker_runtime_lock.lock_owner = ?""",
                ("main", worker_id, expires, now_iso, now_iso, worker_id)
            )
            conn.commit()
            ...
            cursor.execute(
                "SELECT lock_owner, lock_expires_at FROM worker_runtime_lock WHERE lock_name = ?",
                ("main",)
            )
            row = cursor.fetchone()
            if row:
                return WorkerLockResult(
                    ...
                    reason=f"lock held by {row['lock_owner']} until {row['lock_expires_at']}"
                )
    ...
    def refresh_worker_lock(self, worker_id: str, lease_seconds: int) -> bool:
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                (expires, now_iso, "main", worker_id)
            )
            conn.commit()
            return cursor.rowcount > 0
